...
Assert-Contains $clientGreat '[Enum.UserInputState.End]' "GreatSword client must handle mouse release for hold-to-cycle."
Assert-Contains $clientGreat 'function Weapon:StopAttack()' "GreatSword client must stop hold cycling on mouse release."
Assert-Contains $clientGreat 'self.ComboStep' "GreatSword client must track sequential swings."
Assert-Contains $clientGreat 'self.QueuedAttack' "GreatSword client must queue clicks during a swing."
Assert-Contains $clientGreat 'self:_Swing(index)' "GreatSword client must play selected sequential swing."
Assert-Contains $clientGreat 'SPECIAL_COOLDOWN' "GreatSword third swing must trigger long cooldown."
...
Assert-Contains $serverGreat 'function Weapon:AttackStart(swingIndex)' "GreatSword server must accept swing index."
Assert-Contains $serverGreat 'self.SwingIndex' "GreatSword server must track active swing index."
Assert-Contains $serverGreat 'GreatSwordSpecial' "GreatSword server must identify third swing special."
Assert-Contains $config 'SPECIAL_COOLDOWN' "GreatSword config must expose third-swing cooldown."
Assert-Contains $config 'SWINGS' "GreatSword config must define sequential swing data."

Assert-Contains $clientHandler 'CanUnequip' "Client weapon handler must respect unequip lock."
Assert-Contains $clientHandler 'Replica:OnSet({ "Hotbar" }' "Client weapon handler must rebuild when loadout hotbar changes."
Assert-Contains $clientHandler 'weaponName == equippedName' "Client weapon handler must only look in Character for the equipped weapon."
...
Assert-Contains $serverHandler 'WeaponSystem._ReloadHotbar(player)' "Server must rebuild live weapon models after accepted loadout edits."
...
Assert-Contains $loadout 'slotAtMouse' "Loadout UI must compute drop target by hotbar geometry."
Assert-Contains $loadout 'GuiService:GetGuiInset()' "Loadout mouse coordinates must match GUI AbsolutePosition."
Assert-Contains $loadout 'trove:Connect(UserInputService.InputEnded' "Loadout drag release must be cleaned up through Trove."
Assert-NotContains $loadout 'table.insert(scope, UserInputService' "Loadout must not keep input connections outside Trove."
Assert-NotContains $loadout 'print(' "Loadout must not keep debug prints."
Assert-Contains $loadout 'DropOutline' "Loadout UI must outline occupied drop targets."
Assert-Contains $loadout 'disableSlotLayout(HotbarUI)' "Loadout hotbar must not let grid layouts squeeze multi-slot weapons."
Assert-Contains $loadout 'child:Destroy()' "Loadout must remove layout objects instead of toggling unsupported Enabled."
...
Assert-Contains $hudHotbar 'disableSlotLayout(HotbarUI)' "HUD hotbar must not let grid layouts squeeze multi-slot weapons."
Assert-Contains $hudHotbar 'child:Destroy()' "HUD hotbar must remove layout objects instead of toggling unsupported Enabled."
Assert-NotContains $hudHotbar 'child.Enabled = false' "HUD hotbar must not toggle unsupported layout Enabled."
...
Assert-Contains $itemTemplate 'Position = props.Position' "ItemTemplate must allow absolute slot placement."
